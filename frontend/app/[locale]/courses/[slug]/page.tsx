...
export default function CourseDetail() {
  ...
  const enrollMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      router.push(
        `/${locale}/courses/${slug}/lessons/${course.lessons?.[0]?.id || course.lessons?.[0]?._id}`,
      );
    },
    onError: (err: any) => {
            ...
            <div className="card bg-base-100 dark:bg-base-200 text-base-content border border-base-300 shadow-2xl rounded-3xl overflow-hidden max-w-md mx-auto">
              ...
              <div className="p-6 space-y-6">
                <div className="flex items-baseline gap-3">
                  ...
                  {course.price > 0 && (
                    <>
                      <span className="text-sm line-through text-base-content/50 font-bold">
                        $99.99
                      </span>
                      <span className="badge badge-success text-[10px] font-black uppercase text-success-content">
                        50% OFF
                      </span>
                    </>
                  )}
                ...
                {checkoutError && (
                  <div className="alert alert-error text-xs font-bold p-3 rounded-xl">
                    {checkoutError}
                  </div>
                )}
                ...
                {isEnrolled ? (
                  ...
                  >
                    <Play className="w-4 h-4 fill-white text-white" /> Resume
                    Curriculum
                  </Link>
                ) : (
                  <button
                ...
                <div className="space-y-3 pt-4 border-t border-base-300">
                  <h4 className="font-extrabold text-xs text-base-content/80 uppercase tracking-wider">
                    This Course includes:
                  </h4>
                  <ul className="text-xs font-bold text-base-content/70 space-y-2.5">
                    <li className="flex items-center gap-2">
                      <CheckCircle2 className="w-4 h-4 text-success" /> 30-Day
                      Money-Back Guarantee
                    </li>
                    <li className="flex items-center gap-2">
                      <BookOpen className="w-4 h-4 text-[#29B2FE]" /> Full
                      lifetime access
                    </li>
                    <li className="flex items-center gap-2">
                      <Smartphone className="w-4 h-4 text-primary" /> Access on
                      mobile and TV
                    </li>
                    <li className="flex items-center gap-2">
                      <Award className="w-4 h-4 text-[#33D190]" /> Certificate
                      of completion
                    </li>
                  </ul>
                </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        <svg
                          ...
                          viewBox="0 0 24 24"
                        >
                          <path d="M9 8H7v3h2v9h3v-9h3.6l.4-3H12V6c0-.9.2-1.2 1.1-1.2H15V2h-2.8C9.5 2 9 3.5 9 5.8V8z" />
                        </svg>
                      </button>
                      <button
                        className="btn btn-ghost btn-circle btn-xs text-base-content/60 hover:text-primary"
          ...
          <div className="flex border-b border-base-300 gap-6 overflow-x-auto">
            {["overview", "syllabus", "reviews"].map((tab) => (
              <button
                ...
                className={`pb-4 text-sm font-extrabold capitalize transition-all relative whitespace-nowrap ${
                  activeTab === tab
            ...
            {activeTab === "overview" && (
              <div className="space-y-6">
                <div className="card bg-base-100 p-8 rounded-3xl border border-base-300 shadow-sm space-y-4">
                  <h3 className="text-xl font-extrabold text-[#1E2A38] dark:text-white">
                    About this course
                  </h3>
                  ...
                  <p className="text-sm sm:text-base text-base-content/75 leading-relaxed font-medium">
                    Our dynamic study circles synchronize curriculum updates
                    automatically. In this course, you will learn the
                    foundations and advanced structures through comprehensive
                    lectures, active programming tasks, and peer-to-peer coding
                    meetups.
                  </p>
                </div>
              </div>
            ...
            {activeTab === "syllabus" && (
              <div className="card bg-base-100 rounded-3xl border border-base-300 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-base-300 bg-base-200/50">
                  <h3 className="font-extrabold text-[#1E2A38] dark:text-white">
                    Course Curriculum
                  </h3>
                </div>
                <div className="divide-y divide-base-300">
                  {course.lessons?.map((lesson: any, idx: number) => (
                    <div
                      ...
                      className="p-5 flex items-center justify-between hover:bg-base-200/40 transition-colors"
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <span className="text-xs text-base-content/50 flex items-center gap-1 mt-1 font-semibold">
                            <Play className="w-3.5 h-3.5" /> Video Lesson (
                      ...
                      {isEnrolled ? (
                        <Link
                          ...
                          className="btn btn-sm btn-ghost text-primary hover:bg-primary/10 font-bold rounded-xl"
                        >
                          Watch
                        </Link>
                      ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                ...
                <div className="card bg-base-100 p-6 rounded-3xl border border-base-300 shadow-sm space-y-4">
                  <div className="text-center space-y-1">
                    <div className="text-5xl font-black text-[#1E2A38] dark:text-white">
                      {averageRating}
                    </div>
                    ...
                    <div className="text-[10px] font-bold text-base-content/50 uppercase">
                    ...
                    {[5, 4, 3, 2, 1].map((stars) => {
                      ...
                      return (
                        <div key={stars} className="flex items-center gap-3">
                          <span className="w-12 text-base-content/70 text-end">
                            {stars} Stars
                          </span>
                          ...
                          <span className="w-6 text-base-content/50 text-start">
                            {count}
                          </span>
                        </div>
                      );
                ...
                <div className="md:col-span-2 card bg-base-100 p-6 rounded-3xl border border-base-300 shadow-sm space-y-6">
                  {reviews.length > 0 ? (
                    reviews.map((r: any) => (
                      <div
                        key={r.id || r._id}
                        className="border-b border-base-300 pb-5 last:border-0 last:pb-0 flex items-start gap-4"
                      >
                        ...
                        <div className="flex-grow space-y-1">
                          ...
                          <p className="text-sm text-base-content/80 font-semibold leading-relaxed pt-1">
                            {r.comment}
                          </p>
                        </div>
                      </div>
          ...
          <div className="card bg-base-100 p-6 rounded-3xl border border-base-300 shadow-sm">
            ...
            <p className="text-xs sm:text-sm text-base-content/75 leading-relaxed font-semibold">
              {course.instructor?.bio ||
                "An expert educator committed to teaching local community learners."}
            </p>
          </div>
        </aside>
      </main>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
          <div className="flex justify-between items-end">
            <div className="space-y-2">
              <h2 className="text-2xl sm:text-3xl font-extrabold text-[#1E2A38] dark:text-white">
                Marketing Articles
              </h2>
              <p className="text-xs sm:text-sm text-base-content/50 font-semibold">
                Read related career advice and industry tips.
              </p>
            </div>
            <Link
              href={lpUrl("/blog")}
              className="text-primary text-xs sm:text-sm font-extrabold hover:underline"
            >
              See all
            </Link>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              ...
              <div
                key={i}
                className="card bg-base-100 rounded-2xl overflow-hidden border border-base-300 flex flex-col hover:shadow-xl transition-shadow duration-300"
              >
                <div className="relative aspect-video bg-base-300">
                  <img
                    src={art.img}
                ...
                <div className="p-5 flex-grow flex flex-col justify-between space-y-4">
                  <div className="space-y-2">
                    <div className="text-[10px] font-bold text-base-content/40 flex items-center gap-1">
                      ...
                      <span className="text-base-content/70 font-extrabold">
                        {art.time}
                      </span>
                    </div>
                    <h3 className="font-extrabold text-sm text-base-content leading-snug line-clamp-1">
                      {art.title}
                    </h3>
                    <p className="text-xs text-base-content/60 font-semibold line-clamp-2 leading-relaxed">
                      {art.desc}
                    </p>
                  </div>
                  <div className="flex items-center justify-between pt-2 border-t border-base-300/40">
                    ...
                    <span className="text-[10px] font-extrabold text-base-content/40 hover:text-primary cursor-pointer transition-colors">
                      Details
                    </span>
                  </div>
                </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            <h2 className="text-3xl font-extrabold text-[#1E2A38] dark:text-white leading-tight">
              ...
              <span className="text-primary">you can do with us</span>
            </h2>
            <p className="text-sm sm:text-base text-base-content/75 leading-relaxed font-semibold">
              The Codefather synchronizes student portals, live reviews,
              developer notes, and schedules to offer physical classroom
              efficacy in a flexible web browser.
            </p>
            <div className="pt-2">
              <Link
                href={lpUrl("/roadmap")}
      ...
      <section className="py-16 bg-[#E2F0EF]/35">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
          <div className="text-center space-y-2">
            ...
            <p className="text-xs sm:text-sm text-base-content/50 font-bold">
              Benefit from local cohort discounts and memberships.
            </p>
          </div>
            ...
            {[
              {
                ...
                desc: "Join as a local mentor to access professional grading tools and classroom inventory manager.",
              },
              {
                discount: "60%",
                label: "FOR STUDENTS",
                desc: "Acquire corporate membership plans to get unlimited Capstone project reviews.",
              },
              {
                discount: "60%",
                label: "FOR TEAMS",
                desc: "Register group study circles to split classroom booking fees and mentor reviews.",
              },
            ].map((deal, i) => (
              <div
                key={i}
                className="card bg-base-100 p-8 border border-base-300/80 shadow-md rounded-2xl flex flex-col justify-between items-center text-center relative overflow-hidden group"
              >
                <div className="absolute top-4 left-4 bg-primary text-primary-content text-[10px] font-black uppercase px-2.5 py-1 rounded-full">
                  {deal.discount}
                </div>
                <div className="space-y-4 pt-4">
                  <h3 className="font-extrabold text-sm text-base-content/40 tracking-wider uppercase">
                    {deal.label}
                  </h3>
                  <p className="text-xs sm:text-sm text-base-content/75 font-semibold leading-relaxed">
                    {deal.desc}
                  </p>
                </div>
                <Link
                  href={lpUrl("/register")}
                  className="btn btn-primary btn-sm rounded-xl font-bold mt-6 text-white px-6"
                >
                  Get Started
                </Link>
              </div>
