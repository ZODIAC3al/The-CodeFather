...
import { MapPin, Save, Video, Calendar, Menu } from "lucide-react";
...
export default function InstructorSchedulePage() {
    ...
    onError: (err: any) => {
      setErrorMsg(
        err.response?.data?.message ||
          (isRTL ? "فشل حجز الموعد" : "Failed to book slot"),
      );
    },
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!title.trim() || !startAt) {
      setErrorMsg(
        isRTL
          ? "عنوان الجلسة ووقت البدء مطلوبان"
          : "Session Title and Start Time are required",
      );
      return;
    }
    const payload: any = {
  ...
  const labelClass =
    "label text-[10px] font-bold text-base-content/40 uppercase tracking-wider";
  const inputClass =
    "input input-bordered input-sm w-full rounded-xl font-semibold text-xs focus:input-primary";
  const selectClass =
    "select select-bordered select-sm w-full rounded-xl font-semibold text-xs focus:select-primary";
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 max-w-[1400px] mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 gap-6">
        ...
        {/* MAIN — 2-col on desktop, stack on mobile */}
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            <div>
              ...
              <p className="text-xs text-base-content/50 font-medium mt-0.5">
                {isRTL
                  ? "إدارة ندوات الفصول الافتراضية ولقاءات المراكز المحلية"
                  : "Manage virtual classroom webinars and local center meetups"}
              </p>
            </div>

            <div className="bg-base-100 rounded-2xl border border-base-300 overflow-hidden shadow-sm p-4 sm:p-6 space-y-4">
                ...
                <div className="space-y-3">
                  {schedule.map((item: any) => (
                    <div
                      ...
                      className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-base-200/50 hover:bg-base-200 rounded-2xl border border-base-300 transition-colors gap-3"
                    >
                        ...
                        <div className="min-w-0">
                          ...
                          {item.isOffline ? (
                            <p className="text-[10px] text-info font-bold mt-1">
                              {isRTL
                                ? `دائرة حضورية: ${item.centerId?.name}، ${item.roomName} (سعة: ${item.capacity})`
                            ...
                            item.roomUrl && (
                              <a
                                ...
                                className="text-[10px] text-success font-bold hover:underline block mt-1"
                              >
                                {isRTL ? "رابط الانضمام" : "Join Room Link"}
                              </a>
                            )
                          )}
                        </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            >
              <h3 className="text-sm font-black text-base-content">
                {isRTL ? "جدولة جلسة" : "Schedule a Session"}
              </h3>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={
                    isRTL
                      ? "مثال: حلقة NextJS Routing المباشرة"
                      : "e.g. NextJS Routing Live Circle"
                  }
              ...
              <div>
                <label className={labelClass}>
                  {isRTL ? "الدورة المرتبطة" : "Related Course Blueprint"}
                </label>
                <select
                ...
                >
                  <option value="">
                    {isRTL ? "بدون ربط بدورة" : "No Course link"}
                  </option>
                  {myCourses.map((c: any) => (
              ...
              <div className="flex items-center justify-between p-3 bg-base-200/50 rounded-2xl border border-base-300">
                <div>
                  <p className="text-xs font-bold text-base-content">
                    {isRTL ? "مجموعة حضورية" : "Physical Cohort"}
                  </p>
                  <p className="text-[9px] text-base-content/40 font-semibold">
                    {isRTL
                      ? "احجز مقعداً في مراكز الفصول المحلية"
                      : "Book a seat in local classroom centers"}
                  </p>
                </div>
                <input
                  type="checkbox"
              ...
              {isOffline ? (
                <>
                  ...
                  <div>
                    <label className={labelClass}>
                      {isRTL ? "اختر الفصل" : "Select Classroom Room"}
                    </label>
                    <select
                      className={selectClass}
                      value={roomName}
                    ...
                    >
                      {classrooms.map((room: any) => (
                        <option key={room.name} value={room.name}>
                          {room.name} ({isRTL ? "أقصى سعة:" : "Max Capacity:"}{" "}
                          {room.capacity})
                        </option>
                      ))}
                    </select>
                ...
                <div>
                  <label className={labelClass}>
                    {isRTL ? "رابط الاجتماع" : "Custom Meeting Video URL"}
                  </label>
                  <input
                    type="url"
