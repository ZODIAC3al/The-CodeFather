...
export default function AdminDashboard() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-base-200" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div
        className={`fixed top-0 z-50 h-full w-64 bg-base-200 p-4 overflow-y-auto transition-transform duration-300 lg:hidden ${sidebarOpen
          ? "translate-x-0"
          : isRTL
            ? "translate-x-full"
            : "-translate-x-full"
        ...
        <main className="flex-1 min-w-0 flex flex-col gap-6">
            ...
            <div className="flex items-center gap-2">
              {pendingCourses?.length > 0 && (
                <div className="badge badge-warning badge-sm font-bold animate-pulse">
                  {tDash("pendingApproval", { count: pendingCourses.length })}
                </div>
              )}
            </div>
          </div>
              ...
              <div className="flex items-start justify-between mb-1">
                <div>
                  ...
                  <p className="text-xs font-bold text-success flex items-center gap-1 mt-0.5">
                    ...
                    {tDash("revenueThisMonth", { amount: 2100 })}
                  </p>
                </div>
                <Link
                ...
                >
                  {tDash("viewReport")} <ChevronRight className="w-3 h-3" />
                </Link>
              </div>
              <p className="text-[10px] text-base-content/40 font-medium mb-4">
                {tDash("revenueLastDays")}
              </p>
            ...
            <div className="lg:col-span-2 bg-base-100 rounded-2xl border border-base-300 p-5 flex flex-col">
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs font-bold text-success flex items-center gap-1 mt-0.5">
                    <TrendingUp className="w-3 h-3" /> {tDash("usersThisMonth")}
                  </p>
                </div>
                <Link
              ...
              <div className="flex-1 flex items-center justify-center gap-3 py-2">
                {(() => {
                  ...
                  return [
                    {
                      label: tDash("student"),
                      pct: Math.round((studentCount / total) * 100) || 72,
                      color: "bg-primary text-primary-content",
                    },
                    {
                      label: tDash("instructor"),
                      pct: Math.round((instrCount / total) * 100) || 20,
                      color: "bg-accent text-accent-content",
                    },
                    {
                      label: tNav("admin"),
                      pct: Math.round((adminCount / total) * 100) || 8,
              ...
              <div className="mt-4 space-y-1">
                {[
                  {
                    label: tDash("physicalCohorts"),
                    val: `${analytics?.physicalCohorts ?? centers?.length ?? 0}`,
                  },
                  {
                    label: tDash("globalPassRate"),
                    val: `${analytics?.globalPassRate ?? 0}%`,
                  },
                  {
                    label: tDash("pendingApprovals"),
                    val: `${pendingCourses?.length ?? 0}`,
                  },
                ].map(({ label, val }) => (
                  <div
                    key={label}
                    className="flex justify-between text-xs font-semibold text-base-content/60"
                  >
                    <span>{label}</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <div className="bg-base-100 rounded-2xl border border-base-300 p-5">
              <p className="text-xs font-bold text-base-content/50 uppercase tracking-wider mb-1">
                {t("dashboardStats.pendingCoursesTitle")}
              </p>
              <p className="text-2xl font-extrabold text-base-content mb-0.5">
                {pendingCourses?.length ?? 0}
              </p>
              <p className="text-[10px] text-base-content/40 font-medium mb-4">
                {t("dashboardStats.awaitingApproval")}
              </p>
              <div className="space-y-2">
                {(pendingCourses?.length
                  ...
                  : [
                    ...
                    {
                      id: "2",
                      title: "Laravel 12 Bootcamp",
                      instructorName: "Sara Ahmed",
                    },
                    {
                      id: "3",
                      title: "Golang Microservices",
                      instructorName: "Khaled Omar",
                    },
                  ]
                )
                  .slice(0, 4)
            ...
            <div className="bg-base-100 rounded-2xl border border-base-300 p-5">
              <p className="text-xs font-bold text-base-content/50 uppercase tracking-wider mb-1">
                {t("dashboardStats.topLearnersTitle")}
              </p>
              <p className="text-2xl font-extrabold text-base-content mb-0.5">
                {leaderboard?.length ?? 0}
              </p>
              <p className="text-[10px] text-base-content/40 font-medium mb-4">
                {t("dashboardStats.byXpScore")}
              </p>
              <div className="space-y-2">
                {(leaderboard?.length
                  ? leaderboard
                  ...
                  .map((entry: any, i: number) => (
                    ...
                    >
                      <div
                        className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black shrink-0 ${i === 0
                      ...
                      <div className="w-16 h-1.5 rounded-full bg-base-300 overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, ((entry.score ?? entry.xp ?? 0) / 3000) * 100)}%`,
                          }}
                        />
                      </div>
                    </div>
            ...
            <div className="bg-base-100 rounded-2xl border border-base-300 p-5 flex flex-col sm:col-span-2 lg:col-span-1">
              <div className="flex items-start justify-between mb-1">
                <div>
                  <p className="text-xs font-bold text-base-content/50 uppercase tracking-wider mb-1">
                    {t("dashboardStats.userSignupsTitle")}
                  </p>
                  <p className="text-2xl font-extrabold text-base-content">
                    {userGrowthChartData.reduce(
                      (a: number, d: any) => a + d.current,
                      0,
                    )}
                  </p>
                  <p className="text-xs font-bold text-error flex items-center gap-1 mt-0.5">
                    <TrendingDown className="w-3 h-3" />{" "}
                    {t("dashboardStats.vsLastPeriod")}
                  </p>
                </div>
                <Link
                  href={`/${locale}/admin/users`}
          ...
          <div className="bg-base-100 rounded-2xl border border-base-300 overflow-hidden">
            <div className="px-5 py-4 border-b border-base-300 flex flex-wrap items-center justify-between gap-3">
              <h2 className="text-sm font-extrabold text-base-content">
                {t("dashboardStats.userManagementTitle")}
              </h2>
              <div className="join flex-wrap">
                {[
                  { key: "ALL", label: tDash("allRoles") },
                  { key: "STUDENT", label: tDash("student") },
                  { key: "INSTRUCTOR", label: tDash("instructor") },
                  { key: "ADMIN", label: tNav("adminRole") },
                ].map((item) => (
                  <button
                    key={item.key}
                    onClick={() => setRoleFilter(item.key)}
                    className={`join-item btn btn-xs font-bold ${roleFilter === item.key ? "btn-primary" : "btn-ghost border border-base-300"}`}
                  >
                    {item.label}
                  </button>
                ))}
                ...
                <tbody className="divide-y divide-base-300">
                  {filteredUsers?.map((u: any) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-5 text-base-content/60 text-xs font-medium">
                        {u.email}
                      </td>
                      <td className="py-3 px-5">
                        <span
                          className={`badge badge-sm font-bold ${ROLE_BADGE[u.role] ?? "badge-ghost"}`}
                        >
                          {tNav(
                            u.role === "ADMIN"
                              ? "adminRole"
                  ...
                  {!filteredUsers?.length && (
                    <tr>
                      <td
                        colSpan={4}
                        className="text-center py-10 text-sm text-base-content/40 font-medium"
                      >
                        {t("dashboardStats.noUsersFound")}
                      </td>
                    </tr>
                  )}
                </tbody>
