...
function MembershipSuccessPage() {
  ...
  const t = useTranslations('membershipSuccess');
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-16 w-full flex-grow flex flex-col justify-center">
        <div className="card-premium p-8 sm:p-12 rounded-3xl border border-base-300 shadow-xl text-center bg-base-100/80 backdrop-blur-md">
          ...
          {plan && !isPlanLoading && (
            <div className="p-6 bg-base-200/50 rounded-2xl border border-base-300/50 max-w-lg mx-auto mb-8 text-left space-y-4">
              <div className="flex gap-4">
                ...
                <div>
                  <span className="text-xs font-bold text-primary tracking-wider uppercase">{t('subscriptionPlan')}</span>
                  <h4 className="font-bold text-base-content text-sm leading-tight mt-0.5">
                    {locale === 'ar' && plan.name === 'Monthly Pass' ? 'الاشتراك الشهري' : locale === 'ar' && plan.name === 'Annual Pass' ? 'الاشتراك السنوي' : plan.name}
                  </h4>
                  <span className="text-xs text-base-content/50 font-medium">${Number(plan.price || 0).toFixed(2)}/{plan.interval === 'month' ? (locale === 'ar' ? 'شهر' : 'month') : (locale === 'ar' ? 'سنة' : 'year')}</span>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-base-300/50 text-xs font-semibold text-base-content/60 space-y-2">
                <div className="flex justify-between">
                  <span>{t('status')}</span>
                  <span className="badge badge-success text-[10px] font-black uppercase tracking-wider text-success-content px-2 py-1">{t('active')}</span>
                </div>
                <div className="flex justify-between">
                  <span>{t('orderId')}</span>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4 max-w-md mx-auto">
            ...
            >
              <ArrowRight className="w-4 h-4" /> {t('browseCourses')}
            </button>
            <button
